...
export default function AIBookingAssistant({ compact = false, defaultOpen = true }) {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: 'Halo, aku AI Booking Assistant. Aku bisa cek jadwal kosong, deteksi bentrok, dan bantu isi form booking.',
      actions: [],
    },
  ])
  ...
  const helperText = useMemo(() => {
    if (loading) return 'Lagi membaca jadwal...'
        ...
        <div className={`${compact ? 'max-h-64' : 'max-h-80'} overflow-y-auto pr-1 space-y-3`}>
          {messages.map((item, index) => (
            <div key={`${item.role}-${index}`} className={item.role === 'user' ? 'text-right' : 'text-left'}>
              <div
                className={`inline-block max-w-[92%] whitespace-pre-line rounded-2xl px-4 py-3 text-sm leading-relaxed ${item.role === 'user'
