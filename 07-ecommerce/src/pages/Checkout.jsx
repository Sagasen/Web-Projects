...
const DEFAULT_WA_NUMBER = '6285742860240'
...
export const Checkout = () => {
  ...
  // Store settings (QRIS image, bank info, WA number)
  const [settings, setSettings] = useState({
    qris_image_url: '',
    bank_name: 'BNI',
    bank_number: '0244501044',
    bank_holder: 'Silvia',
    wa_number: DEFAULT_WA_NUMBER
  })
  ...
  const fetchSettings = async () => {
    try {
      ...
      setSettings(prev => ({
        ...
        bank_number: map.bank_number || prev.bank_number,
        bank_holder: map.bank_holder || prev.bank_holder,
        wa_number: map.wa_number || prev.wa_number
      }))
    } catch (e) {
      // Non-fatal: fall back to default values above
  ...
  const requiresProof = paymentMethod === 'qris' || paymentMethod === 'transfer_bank'
  ...
  const handleSubmit = async () => {
    ...
    if (requiresProof && !proofUrl) {
      showToast('Upload bukti pembayaran dulu!', 'error')
      return
    }
    ...
    try {
      ...
      setTimeout(() => {
        window.open(`https://wa.me/${settings.wa_number || DEFAULT_WA_NUMBER}?text=${encodeURIComponent(waMsg)}`, '_blank')
        navigate('/orders')
      }, 800)
  ...
  const generateWAMessage = ({ orderNumber, customerName, customerPhone, address, cart, subtotal, paymentMethod, note }) => {
    ...
    const payLabel =
      paymentMethod === 'tunai' ? 'Tunai / COD' :
      paymentMethod === 'qris' ? 'QRIS / E-Wallet' :
      'Transfer Bank'
    const noteSection = note ? `\nCatatan Tambahan:\n"${note}"\n` : ''

    return `PESANAN BARU - BAKOEL UMPLUK
----------------------------------------
Detail Pelanggan:
Nama    : ${customerName}
No. HP  : ${customerPhone}
...
${noteSection}----------------------------------------
Mohon ditunggu konfirmasinya untuk total ongkir dan kesediaan barang ya kak. Terima kasih!`
  }
        ...
        <div className="checkout-grid">
          ...
          <div>
            ...
            <div className="checkout-form-card mb-6">
              ...
              <div className="payment-options">
                ...
                >
                  ...
                  <div className="payment-option-label">Tunai</div>
                </label>
                <label
                ...
                >
                  ...
                  />
                  <div className="payment-option-icon">📱</div>
                  <div className="payment-option-label">QRIS / E-Wallet</div>
                </label>
                <label
                  className={`payment-option ${paymentMethod === 'transfer_bank' ? 'selected' : ''}`}
                ...
                >
                  ...
                  />
                  <div className="payment-option-icon">🏦</div>
                  <div className="payment-option-label">Transfer Bank</div>
                </label>
              </div>
              ...
              <div className={`qr-container ${paymentMethod === 'qris' ? 'visible' : ''}`} id="qr-section">
                ...
                <img
                  src={settings.qris_image_url || '/payment-qr.png'}
                  alt="QRIS Bakoel Umpluk"
                  className="qr-img"
                />
                <p className="qr-note">
              ...
              {paymentMethod === 'transfer_bank' && (
                <div className="bank-info-box mt-4">
                  <p style={{ fontWeight: 700, color: 'var(--primary-dark)', marginBottom: 'var(--space-2)' }}>
                    Transfer ke Rekening
                  </p>
                  <div className="bank-info-name">{settings.bank_name}</div>
                  <div className="bank-info-number">{settings.bank_number}</div>
                  <div className="bank-info-holder">a.n. {settings.bank_holder}</div>
                </div>
              )}

              {/* Shared upload area for QRIS & Transfer Bank */}
              {requiresProof && (
                <>
                  <div
                    ...
                    {!previewUrl ? (
                      <div id="upload-placeholder">
                        ...
                        <p style={{ fontSize: 'var(--text-xs)', color: 'var(--gray-400)', marginTop: '4px' }}>
                          Klik atau pilih foto bukti transfer/pembayaran di sini
                        </p>
                      </div>
                    ) : (
            ...
            <button
              ...
              disabled={submitting || (requiresProof && !proofUrl)}
            >
